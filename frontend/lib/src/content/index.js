  ...
  for (let i = 0; i < sparklesCount; i++) {
    ...
    sparkle.animate(
      [
        ...
        {
          transform: `translate(${randomNumberBetween(
            -100,
            100
          )}px, ${randomNumberBetween(-100, 100)}px)`,
          opacity: 1,
        },
      ],
      {
    ...
    sparkle.animate([{ opacity: 1 }, { opacity: 0 }], {
...
async function run() {
  ...
  const lettersContainer = container.querySelector(
    "#summarize__letters-container"
  );
  ...
  const sparklesContainer = container.querySelector(
    "#summarize__sparkles-container"
  );
  const sparkleCssClass = Array(
    "sumz-fill-sky-600",
  ...
  for (let i = 0; i < sparklesCount; i++) {
    ...
    const sparklePath = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    const sprinkleColor =
      sparkleCssClass[Math.floor(Math.random() * sparkleCssClass.length)];
    sparkleSvg.classList.add(sprinkleColor);
    sparklePath.setAttribute(
      "d",
